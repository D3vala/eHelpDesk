...
async function getTickets() {
    ...
    // If not logged in, return empty — each student should only see their own tickets
    if (!userEmail) {
        return [];
    }
...
function applyFiltersAndRender() {
    ...
    let tickets = allTicketsCache.slice();
    ...
    if (searchTerm) {
        tickets = tickets.filter(t =>
            (t.ticket_id && String(t.ticket_id).toLowerCase().includes(searchTerm)) ||
...
function updateStats(tickets) {
    ...
    const pendingCount = tickets.filter(t => t.status === 'in_progress' || t.status === 'open').length;
    const closedCount  = tickets.filter(t => t.status === 'resolved').length;

    const c1 = document.getElementById('stat-all');
    const c2 = document.getElementById('stat-pending');
    const c3 = document.getElementById('stat-closed');
